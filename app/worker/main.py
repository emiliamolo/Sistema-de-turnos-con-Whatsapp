...
def _show_date_buttons(customer_phone, redis_client, redis_state_key, state, prefix):
    today = date.today()
    dates = []
    ...
    if not dates:
        send_whatsapp_buttons(customer_phone, "No hay fechas disponibles en el futuro cercano.",
                              ["Volver al inicio"])
        return

    date_buttons = [d.strftime("%d/%m (%a)") for d in dates]
    send_whatsapp_buttons(customer_phone, f"{prefix} ¿Qué día preferís?", date_buttons)
    state["phase"] = "WAITING_DATE_SELECTION"
    state["available_dates"] = [d.isoformat() for d in dates]
    redis_client.setex(redis_state_key, STATE_EXPIRATION, json.dumps(state))


def _match_date(message_text, available_dates):
    for d_str in available_dates:
        d = date.fromisoformat(d_str)
        if message_text == d.strftime("%d/%m (%a)").lower():
            return d
    return None
...
def process_message(data: dict, redis_client):
    ...
    try:
        ...
        # --- CASO 5: DATE SELECTION ---
        if phase == "WAITING_DATE_SELECTION":
            available_dates = state.get("available_dates", [])
            selected_date = _match_date(message_text, available_dates)
            if not selected_date:
                date_buttons = [date.fromisoformat(d).strftime("%d/%m (%a)") for d in available_dates]
                send_whatsapp_buttons(customer_phone, "Por favor seleccioná una fecha de la lista:",
                                      date_buttons)
                return

            slots = get_available_slots(db, state["service_id"], selected_date,
                                         staff_id=state.get("staff_id"))
            if not slots:
                send_whatsapp_buttons(
                    customer_phone,
                    f"No hay horarios disponibles para el {selected_date.strftime('%d/%m')}. ¿Querés elegir otra fecha?",
                    ["Elegir otra fecha", "Volver al inicio"],
                )
                state["phase"] = "WAITING_DATE_SELECTION_RETRY"
                redis_client.setex(redis_state_key, STATE_EXPIRATION, json.dumps(state))
                return

            displayed = slots[:3]
            slot_buttons = [datetime.fromisoformat(s["time"]).strftime("%H:%M") for s in displayed]
            send_whatsapp_buttons(customer_phone,
                                  f"Horarios disponibles para el {selected_date.strftime('%d/%m')}:",
                                  slot_buttons)
            state["phase"] = "WAITING_TIME_SELECTION"
            state["selected_date"] = selected_date.isoformat()
            state["slots"] = displayed
            redis_client.setex(redis_state_key, STATE_EXPIRATION, json.dumps(state))
            return

        # --- CASO 5b: DATE SELECTION RETRY ---
        if phase == "WAITING_DATE_SELECTION_RETRY":
            matched = ai_or_exact(message_text, ["Elegir otra fecha", "Volver al inicio"], classifier, "reintento de fecha")
            if matched == "Elegir otra fecha":
                _show_date_buttons(customer_phone, redis_client, redis_state_key, state,
                                   f"Elegiste: {state.get('service_name', 'el servicio')}.")
            elif matched == "Volver al inicio":
                _show_main_menu(customer_phone, redis_client, redis_state_key)
            else:
                send_whatsapp_buttons(customer_phone, "Por favor, seleccioná una opción:",
        ...
        # --- CASO 8: BOOKING ACTION (Reprogramar / Cancelar desde Mis Turnos) ---
        ...
        # --- CASO 9: BOOKING SELECTION (para reprogramar/cancelar un turno específico) ---
        ...
        # --- CASO 10: RESCHEDULE STAFF SELECTION ---
        if phase == "WAITING_RESCHEDULE_STAFF_SELECTION":
            ...
            state["staff_name"] = selected_staff["name"]
            send_whatsapp_buttons(customer_phone,
                                  f"Reprogramar: {state.get('reschedule_service_name', 'el servicio')} con {selected_staff['name']}. Que semana preferis?",
                                  ["Esta semana", "La proxima", "En 2 semanas"])
            state["phase"] = "WAITING_WEEK_SELECTION"
            redis_client.setex(redis_state_key, STATE_EXPIRATION, json.dumps(state))
            return

        # --- CASO 11: RESCHEDULE DATE SELECTION ---
        if phase == "WAITING_RESCHEDULE_DATE_SELECTION":
            available_dates = state.get("available_dates", [])
            selected_date = _match_date(message_text, available_dates)
            if not selected_date:
                date_buttons = [date.fromisoformat(d).strftime("%d/%m (%a)") for d in available_dates]
            ...
            booking = dbs.get_booking(db, state.get("reschedule_booking_id"))
            if not booking:
                send_whatsapp_buttons(customer_phone, "El turno original ya no existe.",
                                      ["Volver al inicio"])
                redis_client.delete(redis_state_key)
                return

            slots = get_available_slots(db, state["reschedule_service_id"],
                                        selected_date, exclude_booking_id=str(booking.id),
                                        staff_id=state.get("staff_id"))
            if not slots:
                send_whatsapp_buttons(
                    customer_phone,
                ...
                state["phase"] = "WAITING_RESCHEDULE_DATE_RETRY"
                redis_client.setex(redis_state_key, STATE_EXPIRATION, json.dumps(state))
                return
            ...
            send_whatsapp_buttons(customer_phone,
                                  f"Horarios disponibles para el {selected_date.strftime('%d/%m')}:",
                                  slot_buttons)
            state["phase"] = "WAITING_RESCHEDULE_TIME_SELECTION"
            state["selected_date"] = selected_date.isoformat()
            state["slots"] = displayed
            redis_client.setex(redis_state_key, STATE_EXPIRATION, json.dumps(state))
            return

        # --- CASO 11b: RESCHEDULE DATE RETRY ---
        if phase == "WAITING_RESCHEDULE_DATE_RETRY":
            matched = ai_or_exact(message_text, ["Elegir otra fecha", "Volver al inicio"], classifier, "reintento de fecha para reprogramar")
            if matched == "Elegir otra fecha":
                _show_date_buttons(customer_phone, redis_client, redis_state_key, state,
                                   f"Reprogramar: {state.get('reschedule_service_name', 'el servicio')}.")
            elif matched == "Volver al inicio":
                _show_main_menu(customer_phone, redis_client, redis_state_key)
            else:
                send_whatsapp_buttons(customer_phone, "Por favor, seleccioná una opción:",
        ...
        # --- CASO 12: RESCHEDULE TIME SELECTION ---
        ...
        # --- CASO 13: RESCHEDULE CONFIRMATION ---
